...
public sealed class BattleshipGame : AuditableBaseEntity<long>
{
    ...
    public static Result<BattleshipGame> Create(string creatorId, string opponentId, DateTimeOffset? createdAt = null)
    {
        ...
        return Result<BattleshipGame>.Success(new BattleshipGame
        {
        ...
        DateTimeOffset? attackedAt = null)
    {
        ...
        var targetShip = opponentShips.FirstOrDefault(ship => ship.PlayerId == opponentId && ship.Occupies(target));
        var isHit = targetShip is not null;
        var attackDate = attackedAt ?? DateTimeOffset.UtcNow;
        var attackResult = BattleshipAttack.Record(Id, attackerId, target, isHit, targetShip?.Id, attackDate);
        ...
        if (opponentShips.Where(ship => ship.PlayerId == opponentId).All(ship => ship.RefreshSunkState(attacksIncludingCurrent, attackDate)))
        {
    ...
    public Result Surrender(string surrenderingUserId, DateTimeOffset? surrenderedAt = null)
    {
        ...
        if (IsFinished())
        {
            return Result.Failure(new DomainError("Battleship.GameAlreadyFinished", "La partida ya finalizo."));
        }
    ...
    public bool IsFinished() => Status is GameStatus.Finished_Winner or GameStatus.Finished_Abandoned;

    public TimeSpan? GetTurnElapsedTime(DateTimeOffset now) => TurnAssignedAt is null ? null : now - TurnAssignedAt.Value;
        ...
        IReadOnlyCollection<BattleshipAttack> existingAttacks)
    {
        ...
        {
            return Result.Failure(new DomainError("Battleship.GameNotInProgress", "La partida no se encuentra en fase de ataque."));
        }

        if (CurrentTurnUserId != attackerId)
        {
            return Result.Failure(new DomainError("Battleship.NotPlayerTurn", "No es su turno de atacar."));
        }

        if (existingAttacks.Any(attack => attack.AttackerId == attackerId && attack.GetTarget() == target))
        {
            return Result.Failure(new DomainError("Battleship.CellAlreadyAttacked", "Ya ha atacado esta coordenada."));
        }
    ...
    private Result ValidateFleet(string playerId, IReadOnlyCollection<BattleshipShip> playerShips)
    {
        ...
        {
            return Result.Failure(new DomainError("Battleship.InvalidFleetCount", "Debe posicionar los 5 barcos requeridos."));
        }

        if (playerShips.Any(ship => ship.PlayerId != playerId))
        {
            return Result.Failure(new DomainError("Battleship.InvalidFleetOwner", "Todos los barcos deben pertenecer al jugador actual."));
        }
        ...
        {
            return Result.Failure(new DomainError("Battleship.InvalidFleetComposition", "La flota debe contener barcos de tamanos 5, 4, 3, 3 y 2."));
        }
        ...
        {
            return Result.Failure(new DomainError("Battleship.OverlappingShips", "Los barcos no pueden superponerse."));
        }
    ...
    private Result EnsurePlayer(string userId)
    {
        ...
        {
            return Result.Failure(new DomainError("Battleship.UserNotInGame", "El usuario no participa en esta partida."));
        }
    ...
    private static List<DomainError> ValidatePlayers(string creatorId, string opponentId)
    {
        ...
        {
            errors.Add(new DomainError("Battleship.CreatorRequired", "El creador de la partida es requerido."));
        }

        if (string.IsNullOrWhiteSpace(opponentId))
        {
            errors.Add(new DomainError("Battleship.OpponentRequired", "El oponente de la partida es requerido."));
        }

        if (!string.IsNullOrWhiteSpace(creatorId) && creatorId.Trim() == opponentId?.Trim())
        {
            errors.Add(new DomainError("Battleship.SelfGameNotAllowed", "No puede iniciar una partida contra usted mismo."));
        }
